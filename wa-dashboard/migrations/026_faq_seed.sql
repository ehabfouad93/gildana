-- The twelve questions people actually ask, seeded into the help centre.
--
-- These are the SAME rows the public landing page renders and the in-app Help centre shows,
-- so an answer can never be right in one place and stale in the other. They are ordinary
-- rows, not code: Admin → Help Content can reword, reorder, hide or delete any of them.
--
-- Seeded ONLY into an empty table. An operator who has already written their own FAQ has
-- made a deliberate choice, and a migration must never overwrite it.
